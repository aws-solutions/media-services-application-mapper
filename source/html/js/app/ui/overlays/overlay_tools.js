...
const fetch_running_pipelines_count = (data) => {
    ...
    } else if (_.has(data, "Destinations"))
        pipelines_count = data.Destinations.length;
...
const has_single_pipeline = (id, data) => {
    ...
    if (data) {
        if (
            _.has(data, "PipelinesRunningCount") &&
            data.PipelinesRunningCount > 1
        )
            return false;

        pipelines_count = fetch_running_pipelines_count(data);

        if (pipelines_count > 1) return false;
    }
...
const set_alarm_text = function (data, drawing, font_size, width) {
    const typeLabel = drawing
        .text(generateText("Active alarms", data))
        .y(alarm_y);
    draw_font_set_position(typeLabel, font_size, width);
};

const set_event_text = function (
    data,
    drawing,
    font_size,
    width,
...
export { set_alarm_text, set_event_text, set_info_text, has_single_pipeline };
