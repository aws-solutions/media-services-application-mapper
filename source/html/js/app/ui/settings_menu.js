...
$("#save_endpoint_connection").on("click", () => {
    try {
        ...
        api_check.ping(endpoint, apiKey).then(async function (response) {
            // test worked
...
$("#cancel_endpoint_connection").on("click", () => {
    ...
    if (current_connection) {
        ...
        api_check.ping(endpoint, api_key).then(function () {
            // test worked
...
$("#advanced_settings_button").on("click", async function () {
    try {
        ...
        const alarm_interval = (await import("../alarms.js")).get_update_interval();
        const event_interval = (await import("../events.js")).get_update_interval();
        const tiles_interval = (await import("./tile_view.js")).get_update_interval();
...
$("#bulk-delete-all-button").click(async function () {
    ...
    confirmation.show(`< p > This action will delete ${diagram_count} diagram${(diagram_count == 1 ? "" : "s")}, ${channel_count} tile${(channel_count == 1 ? "" : "s")}, and all alarm subscriptions in the tool.The browser will reload after completion.Continue ?</p > `, async function () {
        await Promise.all([(await import("./layout.js")).delete_all(), (await import("../channels.js")).delete_all(), (await import("../alarms.js")).delete_all_subscribers()]);
        window.location.reload();
    });
});
