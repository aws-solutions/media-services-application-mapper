...
const alarms_tabulator = new Tabulator("#subscribe_to_alarms_modal_alarm_selection", {
...
const nodes_tabulator = new Tabulator("#subscribe_to_alarms_modal_selected_items", {
...
const update_progress_message = function () {
    ...
    const html = progress_bar_html(progress_value, `Loading alarms (${progress_duration_seconds}s)`, "bg-warning");
...
const populate_alarms_from_region = function (region) {
    ...
    alarms.all_alarms_for_region(region).then(function (response) {
        end_progress_message();
        const html = progress_bar_html(100, `${response.length} alarm${response.length != 1 ? "s" : ""} in this region`, "bg-success");
        set_progress_message(html);
...
$("#subscribe_to_alarms_save").on("click", function () {
    // const diagram = diagrams.shown();
    const node_ids = JSON.parse($("#subscribe_to_alarms_modal").attr("data-node-ids"));
    const promises = [];
