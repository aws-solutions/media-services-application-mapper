...
export const update = () => {
    const items = [];
    return new Promise((resolve, reject) => {
        Promise.all([update_channels(items), update_inputs(items), update_multiplexes(items), update_link_devices(items)]).then(() => {
            resolve(items);
        }).catch((error) => {
            reject(error);
        });
...
export const module_name = "MediaLive Inputs, Channels, Multiplexes, Elemental Link";
