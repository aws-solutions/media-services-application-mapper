...
define(["jquery", "app/server", "app/connections", "app/regions", "app/model", "app/ui/svg_node"],
    function($, server, connections, region_promise, model, svg_node) {
        ...
        var update_link_devices = function(regionName) {
            var current = connections.get_current();
            var url = current[0];
            var api_key = current[1];
            return new Promise((resolve, reject) => {
                server.get(`${url}/cached/link-device/${regionName}`, api_key).then((devices) => {
            ...
            region_promise()
                .then(regions => {
                    ...
                    for (let region_name of regions.get_selected()) {
                        ...
                        promises.push(update_link_devices(region_name));
                    }
