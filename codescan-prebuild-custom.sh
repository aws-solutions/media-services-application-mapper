...
scan_npm() {
    ...
    viperlight scan -t $folder_path -m node-npmaudit -m node-npm6audit
    rc=$?
    if [ $rc -eq 0 ]; then
        echo SUCCESS
    elif [ $rc -eq 42 ]; then
...
scan_py() {
    ...
    echo -----------------------------------------------------------
    viperlight scan -m python-piprot -m python-safety
    rc=$?
    if [ $rc -eq 0 ]; then
        echo SUCCESS
    elif [ $rc -eq 42 ]; then
...
pip install piprot safety pip-licenses bandit

# Runs piprot and safety if there is any requirements.txt
